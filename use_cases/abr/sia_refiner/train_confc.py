...
RANDOM_SEED = 42
MODEL_DIR = ADG_SKYNET_ROOT_ADDR + '/use_cases/abr/sia_refiner/models'  # Model directory path

env = ABREnv()
actor = network.Network(state_dim=S_DIM, action_dim=A_DIM,
                            learning_rate=ACTOR_LR_RATE).to('cuda')
...
for epoch in range(1, TRAIN_EPOCH+1):
    ...
    for step in range(TRAIN_SEQ_LEN):
        ...
        action_prob = actor.predict(
                    torch.tensor(np.reshape(obs, (1, S_DIM[0], S_DIM[1]))).to('cuda'))
    ...
    if epoch % MODEL_SAVE_INTERVAL == 0:
        ...
        placeholder_path = os.path.join(MODEL_DIR, f'confc4_model_t3_ep_{epoch}_temp.pth')
        ...
        final_path = os.path.join(MODEL_DIR, f'confc4_model_t3_ep_{epoch}_{test_score:.3f}.pth')
...
plt.grid(True)
plt.legend()
plt.savefig(ADG_SKYNET_ROOT_ADDR + '/pensive/pensive-gamma/plots/score_vs_epoch.png', format='png')
plt.close()
...
plt.grid(True)
plt.legend()
plt.savefig(ADG_SKYNET_ROOT_ADDR + '/pensive/pensive-gamma/plots/entropy_vs_epoch.png', format='png')
plt.close()