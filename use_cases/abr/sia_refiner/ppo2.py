...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fcbw_actor = nn.Linear(self.s_dim[1], FEATURE_NUM)
    ...
    def forward(self, inputs):
        ...
        split_bw = F.relu(self.fcbw_actor(inputs[:, 4:5, :]).view(-1, FEATURE_NUM))
        split_4 = F.relu(self.conv3_actor(inputs[:, 5:6, :self.a_dim]).view(-1, FEATURE_NUM))
        split_5 = F.relu(self.fc3_actor(inputs[:, 6:7, -1]))

        merge_net = torch.cat([split_0, split_1, split_2, split_3, split_bw, split_4, split_5], 1)
...
class Network(nn.Module):
    ...
    def train(self, s_batch, a_batch, p_batch, v_batch, epoch):
        s_batch = torch.from_numpy(s_batch).to(torch.float32).to('cuda')
        a_batch = torch.from_numpy(a_batch).to(torch.float32).to('cuda')
        p_batch = torch.from_numpy(p_batch).to(torch.float32).to('cuda')
        v_batch = torch.from_numpy(v_batch).to(torch.float32).to('cuda')
    ...
    def load_model(self, nn_model, weights_only=False):
        """Load saved model parameters for Actor and Critic networks."""
        actor_model_params, critic_model_params = torch.load(nn_model, weights_only=weights_only)
