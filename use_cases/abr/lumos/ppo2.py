...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.conv4_actor = nn.Linear(PREDICTION_LENGTH, FEATURE_NUM)
    ...
    def forward(self, inputs):
        ...
        split_4 = F.relu(self.conv4_actor(inputs[:, 4:5, :PREDICTION_LENGTH]).view(-1, FEATURE_NUM))
        split_5 = F.relu(self.conv3_actor(inputs[:, 5:6, :self.a_dim]).view(-1, FEATURE_NUM))
        split_6 = F.relu(self.fc3_actor(inputs[:, 6:7, -1]))

        merge_net = torch.cat([split_0, split_1, split_2, split_3, split_4, split_5, split_6], 1)
