...
class Symbolizer:
    def __init__(
        ...
        self.special_kpis = []
        # List of KPIs that use the trend-based predicate
        self.trend_based_kpis = ["dl_tput", "dl_delay"]
    ...
    def _initialize_kpi_symbols(self, curr_step_dict: dict):
        ...
        for kpi in self.kpis_list:
            ...
            if kpi in self.trend_based_kpis:
                kpi_symbolic_representation[kpi] = self._get_initial_kpi_symb_trend(curr_value[-1], kpi)
                # kpi_symbolic_representation[kpi] = self._get_initial_kpi_symb_trend(np.array(curr_value).mean(), kpi)
            else:
    ...
    def _get_initial_kpi_symb_trend(self, curr_value, kpi_name):
        """
        Generate KPI symbolic representation for the first timestep.
        ...
        predicate = 'const'
        trend = 'same'
        return f"{predicate}({kpi_name}, {trend}, {category})"
    ...
    def _calculate_kpi_symbolic_state(self, curr_step_dict: dict, prev_step_dict: dict):
        ...
        for kpi in self.kpis_list:
            ...
            if kpi in self.trend_based_kpis:
                trend = self._get_trend(curr_value)
                kpi_symbolic_representation[kpi] = self._get_kpi_symb_trend(curr_value[-1], prev_value[-1], kpi, trend)
                # kpi_symbolic_representation[kpi] = self._get_kpi_symb_trend(np.array(curr_value).mean(), np.array(prev_value).mean(), kpi, trend)
            else:
    ...
    def _get_kpi_symb(self, curr_value, prev_value, kpi_name):
        ...
        '''
        # TODO: change back to using percentage for predicate !!!
    ...
    def _get_kpi_symb_trend(self, curr_value, prev_value, kpi_name, trend):
        '''
        Calculate the symbolic representation of KPI changes.
        Args:
            curr_value (float): The current value of the KPI.
            prev_value (float): The previous value of the KPI.
            kpi_name (str): The name of the KPI.
            trend (str): The trend of the KPI.
        Returns:
            str: A string representing the symbolic representation of the KPI change.
        '''
        # TODO: change back to using percentage for predicate !!!
    ...
    def _get_trend(self, time_series):
        """
        Get the trend of a KPI based on the windowed values.

        :param time_series: List of historical KPI values.
        :return: A string representing the trend ('Up', 'Down', 'Same').
        """
    ...
    def _add_timestep_kpi_data_to_approximator(self, timestep_dict):
        ...
        for kpi_name in self.kpis_list:
            if kpi_name in self.trend_based_kpis:
                # self.quantile_manager.partial_fit(kpi_name, [np.array(timestep_dict[kpi_name]).mean()])
                self.quantile_manager.partial_fit(kpi_name, [timestep_dict[kpi_name][-1]])
            else:
