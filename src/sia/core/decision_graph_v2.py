            ...
            for _, _, edge_data in outgoing_edges:
                if 'actions' in edge_data:
                    for action, action_data in edge_data['actions'].items():
                        action_expected_reward = edge_data.get('probability', 0.0) * action_data.get('probability', 0.0) * action_data.get('mean_reward', 0.0)
                        
                        if action == current_action:
                            action_exists = True
                            # current_action_reward = action_data.get('mean_reward', 0.0)
                            current_action_reward = action_expected_reward
                        elif action_expected_reward > best_alternative_reward and self._check_action_compatibility(current_bitrate, action):
                            best_alternative_action = action
                            best_alternative_reward = action_expected_reward
